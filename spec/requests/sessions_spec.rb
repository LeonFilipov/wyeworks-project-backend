RSpec.describe "SessionsController", type: :request do
    ...
    describe 'GET #authorization_needed' do
      it 'returns an authorization message' do
        get "/authorization", headers: { "Authorization" => token }
        expect(response).to have_http_status(:ok)
        expect(JSON.parse(response.body)["message"]).to include("You are authorized")
      end
    end
  end
